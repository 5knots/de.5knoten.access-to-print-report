...
{if $caseCustomFields}
  {foreach from=$caseCustomFields item=group}
      ...
      {* added by katharina on 01-10-20 *}
      {if $group.title eq 'Reflexionen Beratungsprozess' || $group.title eq 'Formalia' || $group.title eq 'Medienanfragen'}
          <h2>{$group.title}</h2>
          <table class ="report-layout">
      ...
      {/if}
    {* added by katharina on 01-10-20 - END *}
